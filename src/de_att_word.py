...
def decomposable_attention(embeddings, maxlen, projection_dim=128, projection_hidden=0, projection_dropout=0.2,
    ...
    q1_encoded = lstm_encoder(q1_embed)
    q2_encoded = lstm_encoder(q2_embed)

    q1_encoded = SpatialDropout1D(0.3)(q1_encoded)
    q2_encoded = SpatialDropout1D(0.3)(q2_encoded)
    ...
    q1_combined = TimeDistributed(Dense(units=256))(q1_combined)
    q2_combined = TimeDistributed(Dense(units=256))(q2_combined)
    ...
    encoder2 = Bidirectional(CuDNNGRU(256, return_sequences=True))
...
config = {
    ...
    'batch_size': 128,
    'epochs': 40,
    'log_path': '../log/de_att_word_f1.csv',
    'model_path': '../models/de_att_word/kf{}.h5',
    'result_path': '../submit/de_att_word_v1.csv'
}
...
def train():
    ...
    for t_idx, v_idx in kf.split(q1, q2, y):
        ...
        # np.save('../prediction/de_att_word/train_kf_{}.npy'.format(split_id), v_pred)
        #
        # y_pred = model.predict([q1_test, q2_test])
        # np.save('../prediction/de_att_word/test_kf_{}.npy'.format(split_id), y_pred)
...
def train_5_round():
    ...
    for round_id in range(1, 6):
        ...
        for t_idx, v_idx in kf.split(q1, q2, y):
            ...
            model = decomposable_attention(embeddings=embed_matrix, maxlen=config['maxlen'])
            model_path = '../models/de_att_word/round_{}_kf_{}.h5'.format(round_id, split_id)
            cb = CB(val_data=([q1_val, q2_val], val_y), model_path=model_path)
            ...
            f1_record.append([split_id, max_f1])
            f1_record_df = pd.DataFrame(f1_record, columns=['fold', 'f1'])
            f1_record_df.to_csv('../log/5round_de_att_word_f1.csv', index=False)

            # 验证集上的预测结果
            v_pred = model.predict([q1_val, q2_val])
            np.save('../prediction/de_att_word/round_{}_train_kf_{}.npy'.format(round_id, split_id), v_pred)

            y_pred = model.predict([q1_test, q2_test])
            np.save('../prediction/de_att_word/round_{}_test_kf_{}.npy'.format(round_id, split_id), y_pred)
