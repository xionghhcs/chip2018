...
def BiMPM(embedding_matrix=None,
    ...
    encoder_layer = Bidirectional(CuDNNGRU(units=256, return_sequences=True))
    # encoder_layer2 = Bidirectional(CuDNNGRU(units=256, return_sequences=True))
    ...
    matching = Dropout(0.25)(matching)

    # matching_max = GlobalMaxPool1D()(matching)

    # Build aggregation layer
    aggregate_layer = Bidirectional(CuDNNGRU(units=256, return_sequences=True))
    aggregation = aggregate_layer(matching)

    # agg_avg = GlobalAvgPool1D()(aggregation)
    agg_max = GlobalMaxPool1D()(aggregation)

    aggregation = agg_max
    # aggregation = Concatenate(axis=-1)([ agg_max, matching_max])
    ...
    model = Model(inputs=[w1, w2], outputs=_out)
...
def train():
    ...
    # q1_train, q1_val, q2_train, q2_val, train_y, val_y = train_test_split(q1, q2, y)
...
def train_5_round():
    ...
    for round_id in range(1, 6):
        ...
        for t_idx, v_idx in kf.split(q1, q2, y):
            ...
            model_path = '../models/bimpm_char/round_{}_kf_{}.h5'.format(round_id, split_id)
            cb = CB(val_data=([q1_val, q2_val], val_y), model_path=model_path)
            model.fit(x=[q1_train, q2_train], y=train_y, batch_size=config['batch_size'], epochs=config['epochs'],
                      validation_data=([q1_val, q2_val], val_y), callbacks=[cb])
            ...
            f1_record.append([split_id, max_f1])
            f1_record_df = pd.DataFrame(f1_record, columns=['fold', 'f1'])
            f1_record_df.to_csv('../log/5round_bimpm_char_f1.csv', index=False)

            # 验证集上的预测结果
            model.load_weights(model_path)
            v_pred = model.predict([q1_val, q2_val])
            np.save('../prediction/bimpm_char/round_{}_train_kf_{}.npy'.format(round_id, split_id), v_pred)

            # 测试集合上预测结果
            y_pred = model.predict([q1_test, q2_test])
            np.save('../prediction/bimpm_char/round_{}_test_kf_{}.npy'.format(round_id, split_id), y_pred)
