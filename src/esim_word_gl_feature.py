...
config = {
    ...
    'batch_size': 256,
    'epochs': 50,
    'log_file': '../log/esim_word_gl_feature_f1.csv',
    'result_file': '../submit/esim_word_gl_feature_v2.csv',
    'model_path': '../models/esim_word_gl_feature/kf_{}.h5'
}
...
def train():
    ...
    gl_vocabulary, gl_embed_matrix = dh.load_embed(file_name='../data/gl_word_vectors.txt')
...
def train_5_round():
    ...
    for round_id in range(1, 6):
        ...
        for t_idx, v_idx in kf.split(q1, q2, y):
            ...
            model_path = '../models/esim_word_gl_feature/roung_{}_fold_{}.h5'.format(round_id, split_id)
            cb = CB(val_data=([q1_val, q2_val, q1_gl_val, q2_gl_val, f_val], v_y), model_path=model_path)
            model.fit(x=[q1_train, q2_train, q1_gl_train, q2_gl_train, f_train], y=t_y, batch_size=config['batch_size'],
                      epochs=config['epochs'],
            ...
            f1_record_df.to_csv('../log/5round_esim_word_gl_feature_f1.csv', index=False)
            print('Kfold [{}] max f1: {}'.format(split_id, max_f1))

            # 保存验证集上的预测结果
            v_pred = model.predict(x=[q1_val, q2_val, q1_gl_val, q2_gl_val, f_train])
            np.save('../prediction/esim_word_gl_feature/round_{}_train_kf_{}.npy'.format(round_id, split_id), v_pred)

            y_pred = model.predict(x=[q1_test, q2_test, q1_test_gl, q2_test_gl, test_feature])
            np.save('../prediction/esim_word_gl_feature/round_{}_test_kf_{}.npy'.format(round_id, split_id), y_pred)
