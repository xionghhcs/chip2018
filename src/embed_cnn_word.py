...
def esim(embeddings, maxlen, dense_dropout=0.5):
    ...
    # encode = Bidirectional(CuDNNGRU(128, return_sequences=True, return_state=True))
    # encode2 = Bidirectional(CuDNNGRU(128, return_sequences=True))
    # encode3 = Bidirectional(CuDNNGRU(128, return_sequences=True))

    # q1_encoded, h, c = encode(q1_embed)
    # q2_encoded, h, c = encode(q2_embed)
    ...
    for kernel_size in filter_list:
        ...
        q1_max_pool.append(Reshape(target_shape=[1, filter_num])(GlobalMaxPool1D()(q1_conv)))
        # q1_avg_pool.append(Reshape(target_shape=[1, filter_num])(GlobalAvgPool1D()(q1_conv)))

        q2_max_pool.append(Reshape(target_shape=[1, filter_num])(GlobalMaxPool1D()(q2_conv)))
...
config = {
    ...
    'batch_size': 256,
    'epochs': 40,
    'log_file': '../log/embed_cnn_word_f1.csv',
    'result_file': '../submit/embed_cnn_word.csv',
    'model_path': '../models/embed_cnn_word/kf_{}.h5'
}
...
def train():
    ...
    for t_idx, v_idx in kf.split(q1, q2, y):
        ...
        np.save('../prediction/embed_cnn_word/train_kf_{}.npy'.format(split_id), v_pred)

        # 保存测试集上的预测结果
        y_pred = model.predict(x=[q1_test, q2_test])
        np.save('../prediction/embed_cnn_word/test_kf_{}.npy'.format(split_id), y_pred)
...
def train_5_round():
    ...
    for round_id in range(1, 6):
        ...
        for t_idx, v_idx in kf.split(q1, q2, y):
            ...
            model_path = '../models/embed_cnn_word/round_{}_kf{}.h5'.format(round_id, split_id)
            cb = CB(val_data=([q1_val, q2_val], v_y), model_path=model_path)
            model.fit(x=[q1_train, q2_train], y=t_y, batch_size=config['batch_size'], epochs=config['epochs'],
                      validation_data=([q1_val, q2_val], v_y), verbose=True, callbacks=[cb])
            ...
            f1_record.append([split_id, max_f1])
            f1_record_df = pd.DataFrame(f1_record, columns=['fold', 'f1'])
            f1_record_df.to_csv('../log/5round_embed_cnn_word_f1.csv', index=False)

            # 保存验证集上的预测结果
            v_pred = model.predict(x=[q1_val, q2_val])
            np.save('../prediction/embed_cnn_word/round_{}_train_kf_{}.npy'.format(round_id, split_id), v_pred)

            # 保存测试集上的预测结果
            y_pred = model.predict(x=[q1_test, q2_test])
            np.save('../prediction/embed_cnn_word/round_{}_test_kf_{}.npy'.format(round_id, split_id), y_pred)
